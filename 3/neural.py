...
class MLP:
    ...
    # x здесь - это уже активированный выход (результат sigmoid), 
    # поэтому формула упрощается до x * (1 - x)
    ...
    # функция обучения реализует Стохастический Градиентный Спуск (SGD)
    def train(self, X, y):
        n_samples = X.shape[0]
        # Создаем массив индексов и перемешиваем их для стохастичности
        indices = np.arange(n_samples)
        np.random.shuffle(indices)

        # Проходим по каждому объекту выборки отдельно
        for i in indices:
            # Берем одну строку и убеждаемся, что она двумерная (1, features)
            xi = X[i].reshape(1, -1)
            yi = y[i].reshape(1, -1)
            
            # Прямой и обратный проход для одного образца
            self.feed_forward(xi)
            self.backward(yi)
    
    # функция предсказания возвращает только выход последнего слоя
    # может работать как с одним образцом, так и с матрицей (для оценки качества)
